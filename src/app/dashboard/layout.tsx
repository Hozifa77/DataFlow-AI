...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const { 
    user, credits, initialCredits, creditsUsed, usagePercent, usageColor,
    notifications, unreadCount, loading, 
    markNotificationRead, markAllRead, logout
  } = useApp();
            ...
            <div className="relative" ref={userDropdownRef}>
              ...
              >
                <div className="hidden md:block text-right">
                  <div className="text-sm font-bold text-gray-900 leading-none mb-1 group-hover:text-[#2E7D32] transition-colors">
                    {user?.user_metadata?.full_name || user?.email?.split("@")[0] || "Account User"}
                  </div>
